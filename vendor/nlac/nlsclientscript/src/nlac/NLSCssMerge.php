...
class NLSCssMerge
{
	...
	protected function replaceImports($content, $baseUrl, $level)
	{
		...
		$content = preg_replace_callback('/^\s*@import[url\(\'"\s]*([^\)\'"]+)[\'\)";\s]*/m', function ($m) use ($baseUrl, $level) {
